...
# ___close_part_git_custom___

# ___open_part_default___
# if [ "$color_prompt" = yes ]; then
...
__conda_setup="$('/home/hanson/anaconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/hanson/anaconda3/etc/profile.d/conda.sh" ]; then
        . "/home/hanson/anaconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/hanson/anaconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
...
bash_prompt() {
	...
	FCG=$(($FONT_COLOR_3+$COLOR))
	BGG=$(($BACKGROUND_G+$BG))
	FEG=$(($TEXTEFFECT_1+$EFFECT))
	...
	format_font TEXT_FORMAT_G $FCG $FEG $BGG
	...
	local PROMT_GITHUB=$"$TEXT_FORMAT_G \$(parse_git_branch) "
...
# \w$(parse_git_branch)
	...
	TSFCG=$(($BACKGROUND_3+$COLOR))
	TSBGG=$(($BACKGROUND_G+$BG))

	TSFC3=$(($BACKGROUND_G+$COLOR))
	TSBG3=$(($DEFAULT+$BG))
	...
	local SEPARATOR_FORMAT_G
	format_font SEPARATOR_FORMAT_1 $TSFC1 $TSBG1
	format_font SEPARATOR_FORMAT_2 $TSFC2 $TSBG2
	format_font SEPARATOR_FORMAT_G $TSFCG $TSBGG
	...
	local SEPARATOR_G=$SEPARATOR_FORMAT_G$TRIANGLE
	...
	*)
	...
	############################################################################
	PS1="$TITLEBAR\n${PROMT_USER}${SEPARATOR_1}${PROMT_HOST}${SEPARATOR_2}${PROMT_PWD}${SEPARATOR_G}${PROMT_GITHUB}${SEPARATOR_3}${PROMT_INPUT}"
